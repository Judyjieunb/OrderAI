...
import { Settings, TrendingUp, LineChart, Shirt, ShoppingCart, BarChart2, Hexagon } from 'lucide-react'
...
import BrandIndexSetup from './components/BrandIndexSetup.jsx'

const steps = [
  { id: 'step0', label: 'Step 0', title: 'Brand Index Setup', desc: '우리 브랜드에서 사용할 목표판매율 기준과 핵심 성과지표(KPI)를 설정합니다.', icon: Settings },
  { id: 'step1', label: 'Step 1', title: 'Target Budget Setup', desc: '전 시즌 마감분석 및 AI제안을 바탕으로 카테고리별 목표 매출과 발주 예산을 설정합니다.', icon: TrendingUp },
  { id: 'step2', label: 'Step 2', title: 'Case Study', desc: '전 시즌 스타일별 실적추이를 주차별로 분석하여, 우리가 놓친 기회비용은 없었는지 점검합니다.', icon: LineChart },
  { id: 'step3', label: 'Step 3', title: 'Style-mapping', desc: '과거 스타일과 신규 스타일 간 유사도 매핑 결과를 확인 하고 확정해주세요. 다음 Step4 발주수량 제안에 유사스타일 실적이 활용됩니다.', icon: Shirt },
  { id: 'step4', label: 'Step 4', title: 'Order Suggest', desc: 'AI 수요 예측과 예산 한도를 반영한 스타일별 발주 추천 수량을 검토합니다.', icon: ShoppingCart },
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
        ...
        <div className="max-w-[1400px] mx-auto px-8">
          <div className="flex items-center space-x-6 overflow-x-auto no-scrollbar">
            <button
              ...
              className="px-3 py-1.5 text-white bg-gradient-to-r from-slate-600 to-blue-600 hover:from-slate-500 hover:to-blue-500 rounded-lg transition-all shadow-md shadow-blue-500/20 hover:shadow-blue-500/40 text-sm font-medium shrink-0"
            >
              Workflow Guide
            </button>
            {steps.map((step) => {
      ...
      <main className="max-w-[1400px] mx-auto px-8 py-8 min-h-[calc(100vh-130px)]">
        ...
        {activeStep === 'step0' && <BrandIndexSetup />}
      ...
      {showGuide && (
        <WorkflowGuide
          ...
          onNavigate={(stepId) => setActiveStep(stepId)}
        />
      )}
    </div>
  )
